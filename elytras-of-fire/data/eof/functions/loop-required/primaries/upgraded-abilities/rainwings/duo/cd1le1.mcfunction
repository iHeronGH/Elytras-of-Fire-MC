...
#Effect Entities
execute as @e[type=area_effect_cloud,tag=rain_cd1le1] at @s positioned ^ ^ ^0.5 positioned ~ ~-1.2 ~ run effect give @e[distance=..0.8] wither 3 2 true

#If Hit a Block or Hits Entity, kill rain_cd1le1
execute as @e[type=area_effect_cloud,tag=rain_cd1le1] at @s unless block ^ ^0.1 ^0.1 #eof:families/projectile-immune run kill @s
execute as @e[type=area_effect_cloud,tag=rain_cd1le1] at @s positioned ^ ^ ^0.45 positioned ~ ~-1.2 ~ if entity @e[type=!#arrows,type=!area_effect_cloud,type=!item,distance=..0.79] run kill @s