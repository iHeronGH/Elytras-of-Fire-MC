...
#Play Particles
execute at @e[type=area_effect_cloud,tag=ice_ri1] run function eof:loop-required/primaries/particles/frost-breath-parts
execute as @e[type=area_effect_cloud,tag=ice_ri1] at @s run tp @s ^ ^ ^0.7

#Place Snow
execute as @e[type=area_effect_cloud,tag=ice_ri1] at @s positioned ^ ^ ^0.7 unless block ~ ~-1 ~ #eof:families/airs if block ~ ~ ~ #eof:families/airs run setblock ~ ~ ~ snow replace
...
execute as @e[type=area_effect_cloud,tag=ice_ri1] at @s unless block ^ ^0.1 ^0.1 #eof:families/projectile-immune run kill @s
