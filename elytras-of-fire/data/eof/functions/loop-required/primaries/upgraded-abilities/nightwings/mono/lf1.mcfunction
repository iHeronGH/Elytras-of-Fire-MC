...
#Play Particles
execute at @e[type=area_effect_cloud,tag=night_lf1] run function eof:loop-required/primaries/particles/flame-breath-parts
execute as @e[type=area_effect_cloud,tag=night_lf1] at @s run tp @s ^ ^ ^0.7

#Effect Entities
execute as @e[type=area_effect_cloud,tag=night_lf1] at @s positioned ^ ^ ^0.7 run fill ^0.5 ^0.55 ^0.35 ^-0.5 ^ ^0.1 fire[age=10] replace #eof:families/airs

#If Hit a Block or Hits Entity, kill night_lf1
execute as @e[type=area_effect_cloud,tag=night_lf1] at @s unless block ^ ^0.2 ^0.15 #eof:families/projectile-immune run kill @s
execute as @e[type=area_effect_cloud,tag=night_lf1] at @s positioned ^ ^ ^0.45 positioned ~ ~-1.2 ~ if entity @e[type=!#arrows,type=!area_effect_cloud,type=!item,distance=..0.75] run kill @s
