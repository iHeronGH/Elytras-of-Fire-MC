...
#Play Particles
execute at @e[type=area_effect_cloud,tag=sea_cd1le2] run function eof:loop-required/primaries/particles/tidal-wave-parts
...
execute as @e[type=area_effect_cloud,tag=sea_cd1le2] at @s unless block ^ ^0.1 ^0.1 #eof:families/projectile-immune unless block ^ ^0.1 ^0.1 water run kill @s
...
execute as @e[type=area_effect_cloud,tag=sea_cd1le2] at @s positioned ^ ^-0.2625 ^ if entity @e[type=tnt,distance=..1.33] run execute as @e[type=tnt] positioned as @s if entity @e[type=area_effect_cloud,tag=sea_cd1le2,distance=..1.33] run fill ~ ~ ~ ~ ~ ~ tnt replace #eof:families/airs
