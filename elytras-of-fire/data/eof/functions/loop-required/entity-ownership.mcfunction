...
execute at @s positioned ~ ~1.2 ~ as @e[type=#eof:throwable-entities,nbt={inGround:0b},distance=..3] at @s positioned ~ ~-1.2 ~ unless entity @a[team=NightWings,scores={premonActive=200..},distance=..3] run tag @s remove premEntity
execute at @s positioned ~ ~1.2 ~ as @e[type=#eof:throwable-entities,tag=!premEntity,distance=..3] run kill @s
