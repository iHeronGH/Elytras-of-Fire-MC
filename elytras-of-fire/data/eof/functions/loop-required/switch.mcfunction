#Switch Modes
	...
	#Add Check Tags
execute if entity @s[tag=!abilSwitch] run tag @s add abilSwitch
...
execute if entity @s[tag=abilSwitch,tag=abilToggOff,tag=!S6,tag=!S7,tag=!S8] run tellraw @s [{"text":"Abilities toggled off!","color":"gray","bold":true,"clickEvent":{"action":"run_command","value":"/trigger abilities"}}]
	...
	#Ability Removal
execute if entity @s[tag=abilSwitch,tag=abilToggOff] run clear @s #eof:abilities{primary:1}
execute if entity @s[tag=abilSwitch,tag=abilToggOff] run clear @s #eof:abilities{secondary:1}
execute if entity @s[tag=abilSwitch,tag=abilToggOff] run clear @s #eof:abilities{tertiary:1}
