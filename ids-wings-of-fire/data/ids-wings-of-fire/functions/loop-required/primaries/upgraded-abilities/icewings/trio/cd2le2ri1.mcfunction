...
execute if entity @s[tag=!CoolDown1,tag=!ArcticCore1,tag=Permafrost2,tag=Icefall1] at @s run summon area_effect_cloud ~ ~1.2 ~ {Tags:["ice_cd2le2ri1"],Particle:"block air",Duration:18,Radius:1f}

#Rotate ice_cd2le2ri1
execute as @e[type=area_effect_cloud,tag=ice_cd2le2ri1] at @s rotated as @p[tag=!CoolDown1] run tp @s ~ ~ ~ ~ ~

#Play Particles
execute at @e[type=area_effect_cloud,tag=ice_cd2le2ri1] run function ids-wings-of-fire:loop-required/primaries/particles/frost-breath-parts
...
execute as @e[type=area_effect_cloud,tag=ice_cd2le2ri1] at @s positioned ^ ^-1.33 ^0.45 if entity @e[type=!#arrows,type=!area_effect_cloud,type=!item,distance=..0.74] run kill @s

#Kill Fire
execute as @e[type=area_effect_cloud,tag=ice_cd2le2ri1] at @s positioned ^ ^ ^1 if block ~ ~0.6 ~ fire run fill ^0.45 ^0.5 ^0.1 ^-0.45 ^-0.5 ^-0.1 air replace fire
