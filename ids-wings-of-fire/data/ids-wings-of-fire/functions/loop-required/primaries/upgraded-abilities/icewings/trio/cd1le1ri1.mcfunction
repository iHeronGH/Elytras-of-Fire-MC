...
execute if entity @s[tag=!CoolDown1,tag=!ArcticCore1,tag=Permafrost1,tag=Icefall1] at @s run summon area_effect_cloud ~ ~1.2 ~ {Tags:["ice_cd1le1ri1"],Particle:"block air",Duration:18,Radius:1f}

#Rotate ice_cd1le1ri1
execute as @e[type=area_effect_cloud,tag=ice_cd1le1ri1] at @s rotated as @p[tag=!CoolDown1] run tp @s ~ ~ ~ ~ ~
...
#Kill Fire
execute as @e[type=area_effect_cloud,tag=ice_cd1le1ri1] at @s positioned ^ ^ ^1 if block ~ ~0.6 ~ fire run fill ^0.45 ^0.5 ^0.1 ^-0.45 ^-0.5 ^-0.1 air replace fire
