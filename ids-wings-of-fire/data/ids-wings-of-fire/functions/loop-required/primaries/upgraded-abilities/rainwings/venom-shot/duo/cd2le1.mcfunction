...
#Execute Venom Shot
execute if entity @s[tag=!CoolDown1,tag=Reaction2,tag=Noxious1,tag=!Potency1] at @s run summon area_effect_cloud ~ ~1.2 ~ {Tags:["rain_cd2le1"],Particle:"block air",Duration:12,Radius:1f}

#Rotate rain_cd2le1
execute as @e[type=area_effect_cloud,tag=rain_cd2le1] at @s rotated as @p[tag=!CoolDown1] run tp @s ~ ~ ~ ~ ~
...
#Effect Entities
execute as @e[type=area_effect_cloud,tag=rain_cd2le1] at @s positioned ^ ^-1.33 ^0.5 run effect give @e[distance=..0.8] wither 3 2 true
