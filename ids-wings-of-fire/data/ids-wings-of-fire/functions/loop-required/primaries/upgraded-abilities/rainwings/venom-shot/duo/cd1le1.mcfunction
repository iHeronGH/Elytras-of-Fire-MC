...
#Execute Venom Shot
execute if entity @s[tag=!CoolDown1,tag=Reaction1,tag=Noxious1,tag=!Potency1] at @s run summon area_effect_cloud ~ ~1.2 ~ {Tags:["rain_cd1le1"],Particle:"block air",Duration:12,Radius:1f}

#Rotate rain_cd1le1
execute as @e[type=area_effect_cloud,tag=rain_cd1le1] at @s rotated as @p[tag=!CoolDown1] run tp @s ~ ~ ~ ~ ~
