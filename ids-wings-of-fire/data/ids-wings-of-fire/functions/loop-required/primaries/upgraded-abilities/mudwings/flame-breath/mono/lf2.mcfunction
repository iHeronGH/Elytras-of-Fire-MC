...
execute if entity @s[tag=!CoolDown1,tag=!Furnace1,tag=Everburn2,tag=!Inferno1] at @s run summon area_effect_cloud ~ ~1.2 ~ {Tags:["mud_lf2"],Particle:"block air",Duration:14,Radius:1f}

#Rotate mud_lf2
execute as @e[type=area_effect_cloud,tag=mud_lf2] at @s rotated as @p[tag=!CoolDown1] run tp @s ~ ~ ~ ~ ~

#Play Particles
execute at @e[type=area_effect_cloud,tag=mud_lf2] run function ids-wings-of-fire:loop-required/primaries/particles/flame-breath-parts
execute as @e[type=area_effect_cloud,tag=mud_lf2] at @s run tp @s ^ ^ ^0.7

#Effect Entities
execute as @e[type=area_effect_cloud,tag=mud_lf2] at @s positioned ^ ^ ^0.7 run fill ^0.5 ^0.55 ^0.35 ^-0.5 ^ ^0.1 fire[age=5] replace #ids-wings-of-fire:families/airs
